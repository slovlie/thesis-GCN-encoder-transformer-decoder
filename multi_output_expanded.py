...
import tempfile  # for temporary file handling in tests
...
import unittest  # for built-in unit tests


def parse_train_test(path):
    """
    Loads the dataset file at `path` and extracts all graphs defined as
    `processX_adj = torch.tensor(...)` and `controlX_Y_adj = torch.tensor(...)`.

    Returns:
        examples: List of dicts with keys:
            'process': 2D Tensor of shape [N, N]
            'controls': List of 2D Tensors, each [K, K]

    Also prints debug information about loaded shapes.
    """
...
class ProcessControlDataset(Dataset):
    ...
    def __init__(self, examples, max_size=None):
        ...
        for ex in examples:
            ...
            # --- NORMALIZE control features --- <<<< ADD
            ctrl_feat_stack = (ctrl_feat_stack - self.ctrl_feat_mean) / self.ctrl_feat_std  # <<< ADD
...
def collate_fn(batch):
    ...
    """
    for i, c in enumerate(ctrls_adj_seq):
        t = c.size(0)
        padded[i, :t] = c
        mask[i, :t] = True
    return procs_adj, padded, mask
    """
...
class SeqAdjPredictor(nn.Module):
    ...
    def __init__(self, max_size, in_feats, d_model=512, nhead=8, layers=3,
        ...
        # I hope you read this, please add to your thesis this part, it is important. This is the uncertainty‐weighting learn function. The parameters for the two feature losses:
        # We'll learn log(σ₁²) and log(σ₂²) so they stay unconstrained.
...
def train_model(model, train_loader, val_loader, epochs, lr, device,
    ...
    optimizer = optim.Adam(model.parameters(), lr=lr, weight_decay=weight_decay)  # <-- weight decay added here
    mse = nn.MSELoss() 
    bce = nn.BCELoss()
    ...
    for epoch in range(1, epochs+1):
        ...
        for batch_idx, (proc_adj, proc_feat, ctrl_adj, ctrl_feat, mask) in enumerate(train_loader, 1):
            ...
            #proc, tgt, mask = proc.to(device), tgt.to(device), mask.to(device)
            ...
            """
            # --- Hard constraint: node 0 should always be type 29  at the last step only---
            ,   find index of last valid timestep for each batch element, mask shape: [B, T]
            # Gather Node 0's predicted type at the last step
            type_preds = []
            for b in range(B):
                type_b = pred_node_feat[b, last_step_idx[b], 0, 0]  # Node 0, type feature
                type_preds.append(type_b)
            node0_type_pred = torch.stack(type_preds)  # shape [B]
            type_constraint_loss = ((node0_type_pred - 29.0) ** 2).mean()
            lambda_type_constraint = 1
            """
            ...
            #batch_idx = torch.arrange(B)
            #node0_type_pred = pred_node_feat[batch_idx, last_step_idx, 0, 0]
            node0_type_pred = torch.stack([
                pred_node_feat[b, last_step_idx[b], 0, 0]  # Node 0, type dim
                for b in range(B)
            ])
            ...
            #constraint_loss = sl_constraint_loss 
            #constraint_loss = sl_constraint_loss + range_violation*lambda_range_constraint
            ...
            # loss_feat1 = ½·exp(-log_var1)·l2_feat1 + ½·log_var1
            loss_feat1 = 0.5 * torch.exp(-model.log_var1) * l2_feat1 \
                        + 0.5 * model.log_var1
            # loss_feat2 = ½·exp(-log_var2)·l2_feat2 + ½·log_var2
            loss_feat2 = 0.5 * torch.exp(-model.log_var2) * l2_feat2 \
                        + 0.5 * model.log_var2           
            
            #mask_expand = mask.view(mask.size(0), mask.size(1), 1, 1).expand_as(tgt)
            #l1 = bce(pred_adj[mask_expand], tgt_sm[mask_expand])
...
def infer(model, proc_adj, proc_feat, max_steps, dataset):
    ...
    with torch.no_grad():
        ...
        for _ in range(max_steps):
            ...
            #emb = model.embed(seq).transpose(0, 1)
            #L = emb.size(0)
            ...
            #normalized_29 = (29.0 - dataset.ctrl_feat_mean[0]) / dataset.ctrl_feat_std[0]
            #feat_raw[0, 0, 0] = normalized_29

            # feat_raw[0, 0, 0] = 29.0    # enforcing constraint: Node 0 = type 29
...
# ... [infer function remains unchanged] ...
...
def main():
    ...
    parser.add_argument('--epochs', type=int, default=300)          # 600<-- extended to 300 epochs
    parser.add_argument('--lr', type=float, default=1e-4)
    parser.add_argument('--label_smoothing', type=float, default=0.05)
    parser.add_argument('--gcn_layers', type=int, default=6)  #4
    parser.add_argument('--residual', action='store_true')
    parser.add_argument('--pos_enc', action='store_true')
    parser.add_argument('--weight_decay', type=float, default=1e-3)  # <-- added, original: 1e-3
    parser.add_argument('--clip_grad', type=float, default=1.0)     # <-- added
    ...
    # — now you can pass λ_constr down into train_model or capture it in the closure
    ...
    # Plot training & validation curves (new visualization)
    plt.figure()
    plt.plot(range(1, args.epochs+1), train_losses, label='Train Loss')  # <-- added label
    plt.plot(range(1, args.epochs+1), val_losses,   label='Val Loss')    # <-- added label
    plt.xlabel('Epoch')
    plt.ylabel('Loss')
...
# -------------------------
    ...
    # -------------------------
    num_display = 5  # <-- how many examples to display
    for ex_idx in range(num_display):  # — MOD: loop over first N examples
        ...
        for j in range(n_ctrl):
            ...
            # --- DENORMALIZE node features before print --- <<<< ADD
            feat_norm = pred_feats[j][:orig.shape[0]].cpu()                         # <<< ADD
            feat_denorm = feat_norm * dataset.ctrl_feat_std + dataset.ctrl_feat_mean # <<< ADD
            feat_denorm = feat_denorm.numpy()                                       # <<< ADD
            ...
            print(feat.round(2))
            print("Predicted Node Features (denormalized):")                         # <<< ADD
            print(feat_denorm.round(2))                                             # <<< ADD            
...
if __name__ == '__main__':
    # Fixed parameters
    d_model = 512
    transformer_layers_options = [2, 4, 6, 8]
    gcn_layer = 6

    all_val_losses = {}

    for transformer_layers in transformer_layers_options:
        # Simulate args (could also use argparse.Namespace)
        class Args:
            data = 'Dataset_general_features.py'
            max_size = None
            batch_size = 16
            epochs = 300
            lr = 1e-4
        ...
        # Reuse your main training logic, extracted to a helper function
        ...
        in_feats = dataset[0][1].shape[-1]
        model = SeqAdjPredictor(
            dataset.max_size,
            in_feats=in_feats,
            d_model=args.d_model,
            nhead=8,
        ...
        #label = f'GCN {args.gcn_layers}'
        #label = f'd_model {args.d_model}'
        label = f'Transformer {args.transformer_layers}'
        all_val_losses[label] = val_losses

    # Plot all validation losses after loop
    ...
    plt.title(f'Validation Loss vs Transformer Layers (GCN Layers={gcn_layer}, d_model={d_model})')
    plt.legend()
    plt.grid(True)
    plt.tight_layout()
    plt.show()
